...
class StationOptimizer:
    ...
    async def _evaluate_batch(self, origin, destination, stations, base_route, fuel_type):
        ...
        matrix = await self.routing.matrix(
            points,
            [0, *station_indexes],
            [*station_indexes, destination_index],
        )
        results = []
        for index, station in enumerate(stations):
            try:
                first_duration = matrix.durations[0][index]
                first_distance = matrix.distances[0][index]
                second_duration = matrix.durations[index + 1][-1]
                second_distance = matrix.distances[index + 1][-1]
    ...
    async def find_best_stations(
        ...
        nearby = filter_stations_near_route(
            base_route.geometry, with_fuel, corridor_km(max_detour_minutes)
        )[: self.max_candidates]
        evaluated = []
        ...
        valid = [
            ...
            if item[1] <= maximum_seconds and item[4] <= maximum_range_meters
        ]
        ...
        logger.info(
            ...
            base_route.duration_seconds, base_route.distance_meters, fuel_type, max_detour_minutes,
            len(stations), len(with_fuel), len(nearby), len(evaluated), len(result_stations), time.monotonic() - started,
        )
        return SearchResult(
            route=RouteResult(duration_minutes=base_route.duration_seconds / 60, distance_km=base_route.distance_meters / 1000, geometry=base_route.geometry),
            stations=result_stations,
            meta=SearchMeta(stations_total=len(stations), candidates_near_route=len(nearby), candidates_evaluated=len(evaluated), results=len(result_stations), fuel_type=fuel_type, max_detour_minutes=max_detour_minutes, autonomy_km=autonomy_km, fuel_data_timestamp=timestamp),
        )
